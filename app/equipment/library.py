...
_TRIP_UNITS: list[TripUnitModel] = [
    ...
    TripUnitModel(
        ...
        notes=(
            ...
            "variante ET. CORREÇÃO (verificação adversarial): a letra 'S' "
            "na referência comercial NÃO indica aplicação Distribuição/"
            "Gerador — é o código de capacidade de interrupção Icu (S = "
            "50 kA@380 Vca), presente ou ausente por classe de Icu do "
            "modelo específico, não por aplicação; hipótese anterior "
            "(S ausente=Distribuição fixo 12×In, S presente=Gerador "
            "ajustável 2-12×In) estava incorreta e foi removida. O que é "
            "confirmado pela fonte: Ii é ajustável de 2 a 12×In numa curva "
            "ÚNICA, com nota explícita do catálogo de que serve tanto para "
            "'proteção de circuitos elétricos de distribuição como para a "
            "proteção de geradores' — mesmo hardware/dial para as duas "
            "aplicações (dial acima já reflete essa faixa 2-12×In "
            "corretamente). As duas tabelas de referência/código do "
            "catálogo tituladas 'Distribuição' e 'Gerador' (com Im=12×In "
            "fixo numa e Im em faixa 2-12×In noutra) existem e foram "
            "citadas na pesquisa original, mas o campo exato do código de "
            "referência que as diferencia não foi confirmado com certeza "
            "— não modelado, [REQUIRES MANUAL DIGITIZATION] se necessário. "
