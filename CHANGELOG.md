...
- `app/equipment/library.py` (aprofundamento FASE 1): nova unidade `WEG-ABW-OCR-TIPO-A` (AZ1/AH1/AC1/AF1..., ajuste de L em dois estágios Iu×Ir, único entre os disparadores do catálogo) e correção da `WEG-ABW-OCR-TIPO-P` existente contra uma revisão mais nova do mesmo catálogo (cód. 50011456, Rev. 23/07-2026 vs. Rev. 5/06-2010 usada originalmente): `S_pickup_Isd` tinha as posições 7 e 9×Ir que não existem na revisão atual (dial corrigido para 8 posições); adicionadas notas sobre as referências alternativas de tr (1.5×Ir e 7.2×Ir), tempos de abertura mín./máx. e a função opcional de fuga à terra (I∆n) não modelada.
...
- `app/equipment/library.py` (aprofundamento FASE 1): `WEG-DWB-DWA-ET-LSI` (disparador eletrônico LSI de 2ª geração ET-C/ETA-C, frames DWB1000/DWA1600, 200-1.600 A) — os valores dos 5 dials (Ir/tr/Isd/tsd/Ii) foram confirmados por **inspeção visual direta** das figuras do catálogo (`pdftoppm` em alta resolução + leitura de imagem), pois a extração de texto do PDF embaralhava os rótulos do gráfico vetorial dos dials rotativos; método usado pela primeira vez nesta sessão após um caso de dado não confiável apenas por texto. Investigado também o catálogo WEG AGW (MCCB de capacidade padrão): confirmado **inteiramente termomagnético** em toda a linha (AGW50-AGW800), sem nenhum disparador eletrônico — nenhuma entrada criada, gap de cobertura fechado por ausência real de dado, não por omissão.
- `app/equipment/library.py` (`WEG-DWB-DWA-ET-LSI`): verificação adversarial com dois métodos independentes — (1) leitura visual CEGA dos 5 dials por um segundo agente, sem acesso à transcrição original, confirmou 100% dos valores (Ir/tr/Isd/tsd/Ii, incluindo identidade entre DWB1000-ET e DWA1600-ET); (2) checagem textual de 7 afirmações das notas, 6 confirmadas. O único mismatch: a hipótese de que a letra "S" na referência comercial (ex. "DWB1000S500-_ET-C") sinalizaria aplicação Gerador (Im ajustável 2-12×In) vs. Distribuição (Im fixo 12×In, sem "S") estava incorreta — "S" é o código de capacidade de interrupção Icu (50 kA@380 Vca), não um marcador de aplicação; removida e substituída pela informação corroborada pela fonte, de que Ii é ajustável 2-12×In numa curva única válida para as duas aplicações (nota explícita do catálogo).
- Eaton LES/MES 310+ (disjuntores Series C, frames LD/MDL) **pesquisado e deferido, não digitalizado**: o PDF original (IL012043EN) não estava mais disponível no scratchpad e as tentativas de re-obtê-lo via `curl` (HTTP/1.1 e HTTP/2) e via `WebFetch` falharam de forma persistente (erro de proxy/servidor, não transitório) — sem a imagem da página não foi possível confirmar com segurança o conjunto completo dos dials "LONG" (tr) e "SHORT" (Isd), que a extração de texto já digitalizada apresentava embaralhados (apenas subconjuntos parciais {10,12,15} e {5,6,7} claramente ancorados, com dígitos adicionais dispersos e não atribuíveis com confiança). Como isso afetaria 2 dos ~5 campos numéricos centrais do dataclass, e a tensão nominal também não é publicada nesta folha de instruções (documento focado na unidade de disparo, não no disjuntor completo), optou-se por **não** criar uma entrada de baixa confiança — decisão alinhada à regra de não inventar dado ausente. Dados com boa confiança que FORAM confirmados nesta investigação (chave combinada tsd/tg letrada J-R, totalmente decodificada e cross-validada pelo texto explicativo; tabela Ir letrada A-H para o maior plugue de sensor de cada frame; Ig discreto {0,2-0,3-0,4-0,6-0,8-1,0}×In, validado por exemplo numérico do próprio texto; override fixo não-ajustável de instantâneo 5620 A/6800 A) ficam registrados apenas nesta nota, candidatos a uma digitalização futura caso o PDF volte a ficar acessível.
...
- `app/equipment/library.py` (`WEG-ABW-OCR-TIPO-P`/`WEG-ABW-OCR-TIPO-A`): verificação adversarial de 2 revisores contra `weg_disjuntores_abertos.txt` (mesma fonte, Rev. 23) revelou que a tabela detalhada do catálogo para o Tipo P usa **faixas contínuas** para G — Ig 0,2~1,0×In (passo 1 A) e tg 0,05~3,0 s —, não os dials discretos de 8 posições assumidos por analogia ao Tipo A; corrigido `G_pickup_Ig`/`G_delay_tg` de `_dial(...)` para `_sr(...)` contínuo no Tipo P (Tipo A permanece discreto, confirmado). Também corrigida a atribuição da função de fuga à terra opcional (I∆n): a lista discreta 0,5/1/2/3/5/10/20/30 A e os tempos de abertura/alarme em ms pertencem ao Tipo A (tabelas separadas para abertura e alarme); o Tipo P usa faixa contínua 0,1~30 A com o MESMO ajuste para abertura e alarme — notas de ambas as entradas reescritas. Tolerância de tr corrigida de "±15% fixo" (valor da tabela simplificada ABWC) para a fórmula real da tabela detalhada ABW: maior valor entre ±10% (Ir<6×In), ±20% (Ir≥6×In) ou ±40 ms.
